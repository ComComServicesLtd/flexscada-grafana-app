  ...
  return {
    ...
    execute: function () {
      ...
      angular.module('grafana.directives').directive('fcsaNumber', function () {
        ...
        isNotDigit = function isNotDigit(which) {
          return which < 45 || which > 57 || which === 47;
        };
        controlKeys = [0, 8, 13];
        isNotControlKey = function isNotControlKey(which) {
          return controlKeys.indexOf(which) === -1;
        };
        hasMultipleDecimals = function hasMultipleDecimals(val) {
          return val != null && val.toString().split('.').length > 2;
        };
        makeMaxDecimals = function makeMaxDecimals(maxDecimals) {
        ...
        makeMaxNumber = function makeMaxNumber(maxNumber) {
          return function (val, number) {
            return number <= maxNumber;
          };
        };
        makeMinNumber = function makeMinNumber(minNumber) {
          return function (val, number) {
            return number >= minNumber;
          };
        };
        makeMaxDigits = function makeMaxDigits(maxDigits) {
          var validRegex;
        ...
        makeIsValid = function makeIsValid(options) {
          var validations;
          validations = [];
