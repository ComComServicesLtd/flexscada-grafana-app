...
System.register([], function (_export, _context) {
      ...
      for (var i = 0; i < points.length; i++) {
        ...
        if (total_count < ratio) {
          ...
          if (total_count === ratio) {
            ...
            var value = min_value;
            if (Math.abs(max_value) > Math.abs(min_value)) value = max_value;
  ...
  _export('dygraph_downsample', dygraph_downsample);
  ...
  _export('Downsample', Downsample);

  function DYDownsample(data, threshold) {
    ...
    for (var i = 0; i < threshold - 2; i++) {
      ...
      // Point a
      var point_a_x = data[a].canvasx * 1,
          // enforce Number (value may be Date)
      ...
      for (; range_offs < range_to; range_offs++) {
        // Calculate triangle area over three buckets
        area = abs((point_a_x - avg_x) * (data[range_offs].canvasx - point_a_y) - (point_a_x - data[range_offs].canvasy) * (avg_y - point_a_y)) * 0.5;
        if (area > max_area) {
          max_area = area;
          max_area_point = data[range_offs];
          next_a = range_offs; // Next a is this b
  ...
  _export('DYDownsample', DYDownsample);

  return {
    setters: [],
    execute: function () {
      _export('floor', floor = Math.floor);
